...
날짜는 YYYY-MM-DD. 버전은 의미 변화 기준. / Dates are YYYY-MM-DD. Versions track meaningful changes. (KO / EN)
...
**Eval 결과 / Eval results:** with_skill 100% (29/29 assertions), without_skill 93.1% (27/29). Δ = +6.9pt. 차이가 집중된 곳: 단일 문체 수렴 감지, AI 클리셰('시사점을 도출') 제거. 차이가 작은 이유: 명확한 slop은 스킬 없이도 Claude가 잘 잡음. / Small delta because obvious slop is caught without the skill too; the skill adds value on subtle patterns.
